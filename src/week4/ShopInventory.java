...
abstract class Item {
    ...
    /**
     * toString that returns the name of the item
     */
...
/**
 * StorageCondition represents a series of behaviours for how that item
...
/**
 * Refrigerate is a kind of StorageCondition where items are refrigerated
...
/**
 * OnSale is a specific kind of price reduction based on
 * seasonal or special sales run by the store.
 */
interface OnSale extends ReducedPrice {
    /**
     * saleCondition takes an array of items and returns
...
class Milk extends Item implements Refrigerate {
    ...
    /** Constructor that takes and sets name, expiry days, price,
    ...
    /**
     * Refrigerate implementation based on current temperature
    ...
    public boolean refrigerate(double currentTemp) {
        return(currentTemp > this.maxRefrigerateTemp);
    }
...
/**
 * Bread is a kind of Item that requires a StorageCondition.
 */
class Bread extends Item implements StorageCondition {
    ...
    /**
     * Constructor that takes and sets name, expiry days, and price.
     */
...
/**
 * Perfume is an Item that requires SecureItem storage.
 */
class Perfume extends Item implements SecureItem {
    ...
    /**
     * Attaching a security tag. Sets item to locked.
     */
    ...
    /**
     * getter for checking whether item is locked.
     */
    boolean getLocked() {
        return(locked);
    }
...
/**
 * EasterSale is a kind of On Sale event
 */
class EasterSale implements OnSale {
    ...
    /**
     * SaleCondition that takes in an array of items and checks
...
public class ShopInventory {
    ...
    /**
     * billItems method that takes an array of items and an OnSale instance.
    ...
    public static double billItems(Item[] items, OnSale Sale) {
        ...
        for (Item item : items) {
            if(item instanceof StorageCondition) {
                System.out.println(item + " (" + item.price + ") (Storage: " + ((StorageCondition) item).storageProcedure() + ")");
            }
            else {
