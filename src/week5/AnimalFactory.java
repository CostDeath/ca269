...
class Duck extends Item {
    private final int maxItemsPermitted = 5;
...
class Swan extends Item {
    private final int maxItemsPermitted = 2;
    public int getMaxItemsPermitted() {
        return maxItemsPermitted;
    }
}
...
class Flamingo extends Item {
    private final int maxItemsPermitted = 3;
    public int getMaxItemsPermitted() {
        return maxItemsPermitted;
    }
}
...
class Dog extends Item {
    private final int maxItemsPermitted = 20;
    public int getMaxItemsPermitted() {
        return maxItemsPermitted;
    }
}
...
public class AnimalFactory<T extends Item & SafetyRegulation>  {
    ...
    T continueProduction(T item) {
        if(item.getMaxItemsPermitted() > this.unitsProduced) {
            this.unitsProduced++;
            return(item);
        }
        return(null);
